...
$data = [
  [
    "title"       => "Addressing Remote Needs",
    "description" => "When a world-recognized supplier of high-quality electrical and mechanical equipment needed attendees from several countries to remotely witness factory acceptance tests and final inspections for topside electrical cabinets, they chose Seebird’s inspection system.",
    "url"         => "#",
  ],
  [
    "title"       => "Use Cases",
...
<div class="bg-gray-50">
  <div class="container mx-auto py-8 px-3 2xl:px-0">
    ...
    <?php foreach ( $data as $key => $item ): ?>
      <div
        class="grid md:grid-cols-2 gap-6 md:gap-10 place-items-center md:place-items-stretch <?php echo $key === ( count( $data ) - 1 ) ? '' : 'mb-12'; ?>"
        data-aos="fade-up"
        data-aos-delay="100"
      >
        <div
          class="flex flex-col justify-center items-center md:items-start <?php echo $key % 2 === 1 ? 'order-2' : 'order-2 md:order-none' ?>">
          <div class="text-3xl lg:text-5xl font-bold mb-3 lg:mb-5"><?php echo $item['title']; ?></div>
          <p
            class="text-center md:text-left lg:text-lg text-gray-500 leading-normal lg:leading-relaxed mb-4 line-clamp-4">
